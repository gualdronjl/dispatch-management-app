...
export default function DispatchList() {
    ...
    const canCreateDispatch = isAdmin() || isOperator();
    ...
    const filtered = dispatches.filter((d) =>
        [shortDispatchCode(d), d.id, d.delivery_point?.address, d.delivery_point?.name, d.delivery_point?.phone, d.driver?.full_name, d.driver?.plate, d.vehicle_plate, d.delivery_point_id, d.created_by, d.status].some((v) =>
                            ...
                            filtered.map((d) => (
                                <Fragment key={d.id}>
                                    <TableRow key={d.id} sx={{ "&:hover": { bgcolor: "rgba(255,255,255,0.02)" } }}>
                                        ...
                                        <TableCell sx={{ ...cellSx, fontWeight: 700, color: "#E2E8F0" }}>
                                            {d.driver?.full_name || "-"}
                                            {(d.vehicle_plate || d.driver?.plate) && <Typography sx={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: "#94A3B8" }}>Placa: {d.vehicle_plate || d.driver?.plate}</Typography>}
                                        </TableCell>
